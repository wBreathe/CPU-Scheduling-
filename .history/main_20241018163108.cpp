...
void AlgorithmA(vector<Process>processes){
    ...
    printInfo("Average STATS: ", ceil(averageCPU / processes.size()), ceil(averageCompletion / processes.size()), ceil(avgWait / processes.size()), ceil(avgTurn / processes.size()), totalSwitch);
}
...
void AlgorithmB(vector<Process>processes){
    ...
    for(int i = 0; i < finished.size(); ++i){
        cout << i + 1 << ":" << endl;
        printInfo("PID: " + std::to_string(finished[i].getPID()), finished[i].getCPUTime(), finished[i].getTimeOfCompletion(), finished[i].getWaitTime(), finished[i].getTurnaroundTime(), finished[i].getNumOfContextSwitching());
    ...
    printInfo("Average STATS: ", ceil(averageCPU / processes.size()), ceil(averageCompletion / finished.size()), ceil(avgWait / finished.size()), ceil(avgTurn / finished.size()), totalSwitch);
...
void AlgorithmC(vector<Process>processes){
    ...
    int quantum = 0,time = 0, counter = 0, runTime = 0;
    double averageCompletion = 0, avgWait = 0, avgTurn = 0, averageCPU = 0, totalSwitch = 0;
